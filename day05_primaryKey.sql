...
---------------------------------------------------------------------------*/
 ...
 (
	ted_id INT,
    urun_id char(13), 
    CONSTRAINT urunler_fk FOREIGN KEY(ted_id) REFERENCES tedarikci(tedarikci_id)
);

/*------------------------------------------------------------------------
"bebeler" isimli bir tablo olusturun. 
Tabloda "id", "isim", "iletisim" field'lari olsun ve "id" yi Primary Key yapin.
...
(
	bebe_id INT,
    puan INT,
    CONSTRAINT fk_bebe_id FOREIGN KEY(bebe_id) REFERENCES bebeler(id)
);  
